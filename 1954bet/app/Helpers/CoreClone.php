...
class CoreClone
{
    ...
    {
        if($wallet->balance_bonus_rollover > 0 && $wallet->balance_bonus > 0){
            if($wallet->total_balance <= 0.5000 || $wallet->balance_bonus_rollover < 0){
                $wallet->update(['balance_bonus_rollover' => 0]);
            }
    
            if($wallet->balance_bonus_rollover > 0) {
                $wallet->decrement('balance_bonus_rollover', $balance);
            }
        }else{
            if($wallet->total_balance <= 0.5000 || $wallet->balance_deposit_rollover < 0){
    ...
    public static function payWithRollover($userId, $changeBonus, $WinAmount): void
    {
        ...
        if($wallet->total_balance <= 0.1000 || $wallet->balance_deposit_rollover < 0){
            $wallet->update(['balance_deposit_rollover' => 0]);
        }
    ...
    private static function PayWin($user, $affiliate, $type, $amount, $changeBonus)
    {
        ...
        \DB::table('debug')->insert(['text' => $changeBonus]);
    ...
    {
        $secretKey = "trevopix.bet"; // Você pode usar env('WEBHOOK_SECRET_KEY') para maior segurança
        $iv = openssl_random_pseudo_bytes(16); // Gerar um IV de 16 bytes
        $encryptedData = openssl_encrypt($data, 'aes-256-cbc', $secretKey, 0, $iv);
        return base64_encode($iv . $encryptedData); // Incluir o IV no início do texto criptografado
    }
    ...
    public static function decryptData($encryptedData)
    {
        ...
        $iv = substr($data, 0, 16); // Extrair os primeiros 16 bytes como IV
        $encryptedData = substr($data, 16); // O restante é o texto criptografado
